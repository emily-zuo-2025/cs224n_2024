...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            # Shift: remove the first element from buffer and push it onto stack
            self.stack.append(self.buffer.pop(0))
        elif transition == "LA":
            # Left Arc: add dependency (head=stack[-1], dependent=stack[-2]), remove stack[-2]
            self.dependencies.append((self.stack[-1], self.stack[-2]))
            self.stack.pop(-2)
        elif transition == "RA":
            # Right Arc: add dependency (head=stack[-2], dependent=stack[-1]), remove stack[-1]
